...
export default function RankDetail() {
    ...
    const fetchTracking = async (showLoading = false) => {
        ...
        try {
            const [{ data: keywordData }, { data: historyData }] = await Promise.all([
                api.get(`/api/keywords/${id}`),
                api.get(`/api/keywords/${id}/history`),
            ]);

            const keyword = keywordData.keyword;
            const rankHistory = (historyData.history || []).map((entry: { position: number | null; checkedAt: string }) => ({
    ...
    const handleRefresh = async () => {
        ...
        setRefreshing(true);
        try {
            await api.post(`/api/keywords/${tracking._id}/check`);
            await fetchTracking();
        } catch (error) {
    ...
    useEffect(() => {
        (async () => await fetchTracking(true))();
    }, [id]);
