...
export default function RankTracker() {
    ...
    const fetchKeywords = async () => {
        try {
            ...
            setAddError(error.response?.data?.message || "Unable to load tracked keywords.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAdd = async (e: React.SubmitEvent) => {
        ...
        try {
            ...
            const { data } = await api.post("/api/keywords", {
                keyword,
                domain: parsedUrl.hostname,
                url: parsedUrl.toString(),
            });

            setKeywords((currentKeywords) => [data.keyword, ...currentKeywords]);
            setShowAddModal(false);
            setNewKeyword("");
            setNewUrl("");
        } catch (error: any) {
            console.error("Add keyword request failed:", error.response?.data || error);

            setAddError(
                error.response?.data?.message ||
                (error.response?.status === 401
                    ? "Your session has expired. Please log in again."
                    : error.message || "Unable to add keyword.")
            );
        } finally {
            setAdding(false);
        }
    ...
    const handleRefresh = async (id: string) => {
        setRefreshing(id);
        try {
            await api.post(`/api/keywords/${id}/check`);
            await fetchKeywords();
        } catch (error: any) {
            setAddError(error.response?.data?.message || "Rank checking is not available yet.");
        } finally {
            setRefreshing(null);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            setKeywords((currentKeywords) => currentKeywords.filter((keyword) => keyword._id !== id));
        } catch (error: any) {
            setAddError(error.response?.data?.message || "Unable to delete keyword.");
        } finally {
            setDeleting(null);
        }
    };
    ...
    const handleToggle = async (id: string) => {
        try {
            const { data } = await api.patch(`/api/keywords/${id}/active`);
            setKeywords((currentKeywords) => currentKeywords.map((keyword) => keyword._id === id ? data.keyword : keyword));
        } catch (error: any) {
            setAddError(error.response?.data?.message || "Unable to change tracking status.");
        }
    };
